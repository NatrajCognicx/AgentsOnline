                ...
                Using memoryStream As New System.IO.MemoryStream()
                    ...
                    If iMMT = 1 Then
                        ...
                        If (headerDr("div_code") = "01") Then
                            cell = HeaderImageCell("~/Logos/mahce_logo.jpg", 141, 95, PdfPCell.ALIGN_CENTER)
                        Else
                            cell = HeaderImageCell("~/Logos/mahce_logo.jpg", 141, 95, PdfPCell.ALIGN_CENTER)
                        End If
                        ' cell.Rowspan = 2
                    ...
                    Else
                        If headerDt.Rows.Count > 0 Then
                            ...
                            phrase = New Phrase()
                            phrase.Add(New Chunk(Convert.ToString("Mahce Reference"), NormalFont))
                            cell = PhraseCell(phrase, PdfPCell.ALIGN_LEFT, 1, False)
                            cell.VerticalAlignment = PdfPCell.ALIGN_TOP
                            If iMMT = 0 Then
                                cell.BackgroundColor = bookingBGcolor
                    ...
                    If serviceDt.Rows.Count > 0 Then
                        ...
                        For Each serviceDr As DataRow In serviceDt.Rows
                            ...
                            If Convert.ToString(serviceDr("ServDetail2Title")) <> "" And Convert.ToString(serviceDr("ServDetail2")) <> "" Then
                                ...
                                tblService.AddCell(cell)
                            End If
                            'added param on 01/08/2020
                            If Convert.ToString(serviceDr("Remarks")) <> "" Then
                                phrase = New Phrase()
                                phrase.Add(New Chunk("Remarks", NormalFont))
                                cell = PhraseCell(phrase, PdfPCell.ALIGN_LEFT, 1, False)
                                cell.VerticalAlignment = PdfPCell.ALIGN_TOP
                                If iMMT = 0 Then
                                    cell.BackgroundColor = serviceBGcolor
                                ...
                                phrase = New Phrase()
                                phrase.Add(New Chunk(Convert.ToString(serviceDr("Remarks")), NormalFont))
                                cell = PhraseCell(phrase, PdfPCell.ALIGN_LEFT, 1, False)
                                cell.VerticalAlignment = PdfPCell.ALIGN_MIDDLE
                                If iMMT = 0 Then
                                    cell.BackgroundColor = serviceBGcolor
...
#End Region
    ...
    Private Shared Function HeaderImageCellMMT(ByVal path As String, ByVal Widthscale As Single, HeightScale As Single, ByVal align As Integer) As PdfPCell
        Dim image As iTextSharp.text.Image = iTextSharp.text.Image.GetInstance(HttpContext.Current.Server.MapPath(path))
