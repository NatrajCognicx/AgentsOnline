...
Partial Class GuestPagenew
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            Try
                'added Saurabh on 18/10/20202
                Dim merchantTransactionId As String = String.Empty
                Dim Status As String = String.Empty
                Dim PaymentId As String = String.Empty
                Dim Message As String = String.Empty
                PaymentStatus.Text = ""

                Dim enckey As String = ConfigurationManager.AppSettings("EnKey").ToString

                If Not Request.QueryString("Status") Is Nothing Then
                    Status = objBLLCommonFuntions.fnDecryption(Request.QueryString("Status"), enckey)
                End If

                If Status <> "" Then
                    If Not Request.QueryString("merchantTransactionId") Is Nothing Then
                        merchantTransactionId = objBLLCommonFuntions.fnDecryption(Request.QueryString("merchantTransactionId"), enckey)
                    End If

                    If Not Request.QueryString("PaymentId") Is Nothing Then
                        PaymentId = objBLLCommonFuntions.fnDecryption(Request.QueryString("PaymentId"), enckey)
                    End If
                    If Not Request.QueryString("Message") Is Nothing Then
                        Message = objBLLCommonFuntions.fnDecryption(Request.QueryString("Message"), enckey)
                    End If
                End If

                If Status = "1" Then
                    divPayNow.Attributes.Add("style", "display:none")
                    divPayLater.Attributes.Add("style", "display:none")
                    If Session("PaymentReverseFlag") = "1" Then
                        divSubmitBooking.Attributes.Add("style", "display:none")
                    Else
                        divSubmitBooking.Attributes.Add("style", "display:block")
                    End If
                    PaymentDetails.Visible = True
                    dvPaymentDetails.Style.Add("display", "block")
                    dvPaymentStatus.Style.Add("display", "block")
                    Dim strSqlQry As String = ""
                    Dim myDS As New DataSet
                    If Not merchantTransactionId Is Nothing Then

                        Dim updateResult As Boolean = objBLLguest.UpdatePaymentStatus(merchantTransactionId, PaymentId, True, Session("GlobalUserName"))

                        strSqlQry = "execute sp_get_paymentRequestDetails '" & merchantTransactionId & "'"
                        ...
                        PaymentAmount.Text = ObjBLLPaymentParameters.PaymentAmount.ToString
                        PayCurrency.Text = ObjBLLPaymentParameters.PaymentCurrency

                        hdnPaymentRequestId.Value = merchantTransactionId
                        hdnPayAmount.Value = ObjBLLPaymentParameters.PaymentAmount
                        hdnPaymentId.Value = PaymentId
                        hdnPayCurrency.Value = ObjBLLPaymentParameters.PaymentCurrency
                    End If
                ElseIf Status = "0" Then
                    Dim updateResult As Boolean = objBLLguest.UpdatePaymentStatus(merchantTransactionId, PaymentId, False, Session("GlobalUserName"))
                    divSubmitBooking.Attributes.Add("style", "display:none")
                ...
                dvColRef.Style.Add("display", "none")
                '*** Danny 12/08/2018
            ...
            End Try
        Else
            If chkColRef.Checked = True Then
    ...
    Private Sub LoadHome()
        ...
        Fillguestnames(Session("sRequestId"))
        ...
        Dim strQuery As String = "select 't' from booking_guest_flightstemp(nolock) where  requestid='" & CType(Session("sRequestId"), String) & "'"
    ...
    Private Sub LoadFields()
        ...
        Else
            '  
            Dim strRequestIdModifyUser As String = ConfigurationManager.AppSettings("RequestIdModifyUser").ToString
            If strRequestIdModifyUser <> "" Then
                Dim RequestIdModifyUser As String() = strRequestIdModifyUser.Split("\")
                If RequestIdModifyUser.Length > 0 Then
                    For i As Integer = 0 To RequestIdModifyUser.Length - 1
                        If Session("GlobalUserName").ToString.ToUpper = RequestIdModifyUser(i).ToUpper Then
                            dvColumbusReference.Visible = True
                            Exit For
                        Else
                            dvColumbusReference.Visible = False
    ...
    Protected Sub dlPersonalInfo_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataListItemEventArgs) Handles dlPersonalInfo.ItemDataBound
        ...
        '    '  lblroomno.Text = "Room#1 : FORTUNE HOTEL DEIRA (FORMELY DEIRA TOWN HOTEL) (2 Adults + 1 Child)"
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            '  Dim dt1 As DataTable = objBLLHotelSearch.GetResultAsDataTable("select case when isnull(shiftto,0)=1 then convert(varchar(10),dateadd(d,1,checkin),111) else convert(varchar(10),checkin,111) end  checkin, convert(varchar(10),checkout,111) checkout from view_booking_hotel_prearr(nolock)  where requestid='" & Session("sRequestId") & "'  and  rlineno=" & Val(lblrlineno.Text))
            ...
            'Dim dt1 As DataTable = objBLLHotelSearch.GetResultAsDataTable("select convert(varchar(10),checkin,111)   checkin,convert(varchar(10),checkout,111) checkout from booking_hotel_detailtemp  where requestid='" & Session("sRequestId") & "'  and  rlineno=" & Val(lblrlineno.Text))
            If dt1.Rows.Count > 0 Then
            ...
            If lblType.Text.Contains("Child") = True Then
                ' dvChildAge.Visible = True
                dvChildAge.Style.Add("display", "block")
                txtChildAge.ReadOnly = False
            Else
                ' dvChildAge.Visible = False
                dvChildAge.Style.Add("display", "none")
            End If


            '   Dim btnshow As Button = CType(e.Item.FindControl("btnshow"), Button)
            ...
            '  btnshow.Attributes.Add("onClick", "javascript:return showhideservices('" + CType(btnshow.ClientID, String) + "','" + CType(divservices.ClientID, String) + "','" + CType(e.Item.ItemIndex, String) + "' )")
            ...
            End If
            'If Session("sLoginType") <> "RO" Then
            '    '  txtVisaPrice.ReadOnly = True
            '    txtVisaPrice.Attributes.Add("onkeydown", "fnReadOnly(event)")
            'Else
            '    ' txtVisaPrice.ReadOnly = False
            '    txtVisaPrice.Attributes.Remove("onkeydown")
            'End If
            BindTittle(lblType.Text, ddlTittle)
            ddlTittle.Text = IIf(Convert.ToString(dtRow("Title")) = "", "", dtRow("Title"))

            ''commented the entire part -- changed by mohamed on 11/09/2018
            'If Val(lblshiftto.Text) = 1 Then ' Modified by abin on 18/04/2018    If Val(lblshiftto.Text) = 1  Then --changed by mohamed on 04/09/2018 removed < 2 condition --And roomno < 2
            '    txtFirstName.Attributes.Add("onkeydown", "fnReadOnly(event)")
            '    txtLastName.Attributes.Add("onkeydown", "fnReadOnly(event)")
            '    txtMiddleName.Attributes.Add("onkeydown", "fnReadOnly(event)")
            '    txtNationality.Attributes.Add("onkeydown", "fnReadOnly(event)")
            '    ddlTittle.Enabled = False
            'End If
...
<System.Web.Services.WebMethod()> _
    ...
    Private Sub ValidateGuestService(ByVal intAuto As Integer) '*** Danny 01/09/2018
        ...
        If ValidatePersonalDetails() Then
            ...
            If dlPersonalInfo.Items.Count > 0 Then
                ...
                Next
            End If
            objBLLguest.GBGuestXml = strBuffer.ToString.Replace("&", "and").Replace("+", "plus")
            objBLLguest.GBServiceXml = strServiceBuffer.ToString.Replace("&", "and").Replace("+", "plus")
            objBLLguest.GBuserlogged = Session("GlobalUserName")
    ...
    Sub SavingGuests()
        ...
        If dlPersonalInfo.Items.Count > 0 Then
            ...
            Next
        End If
        objBLLguest.GBGuestXml = strBuffer.ToString.Replace("&", "and").Replace("+", "plus") 'modified by abin on 20201026
        objBLLguest.GBServiceXml = strServiceBuffer.ToString.Replace("&", "and").Replace("+", "plus")
        objBLLguest.GBuserlogged = Session("GlobalUserName")
    ...
    Sub sendemailtohotel(ByVal partycode As String, ByVal amended As String, ByVal cancelled As String)
        ...
        Try
            ...
            param(0) = New SqlParameter("@bookingCreatedBy", CType(Session("sLoginType"), String))
            param(1) = New SqlParameter("@requestId", CType(requestid, String))
            Dim dtEmailConfigure As DataTable = objclsUtilities.GetDataTable("sp_get_emailConfiguration_hotel", param)
            Dim roEmailRequired As Boolean = False
            Dim hotelRequest As Boolean = False
            Dim ccEmailRequired As Boolean = False
            Dim commonEmailId As String = ""
            If dtEmailConfigure.Rows.Count > 0 Then
                roEmailRequired = CType(dtEmailConfigure(0)("roEmail"), Boolean)
                hotelRequest = CType(dtEmailConfigure(0)("hotelRequest"), Boolean)
                ccEmailRequired = CType(dtEmailConfigure(0)("CcEmail"), Boolean)
            ...
            If hotelDt.Rows.Count > 0 Then
                ...
                Else
                    ...
                    '' Added shahul 04/12/18
                    strMessage += "<p class='MsoNormal' style='margin: 0'><span style='font-family:calibri,sans-serif;color:#3E3F4B'>For Availability Updates  :<span style='font-family: calibri, arial, helvetica, sans-serif; color: #0033ff; text-decoration: underline;'>stopsale@Mahce.net</span></span></p>"
                    strMessage += "<p class='MsoNormal' style='margin: 0'><span style='font-family:calibri,sans-serif;color:#3E3F4B'>For Rate related updates  :<span style='font-family: calibri, arial, helvetica, sans-serif; color: #0033ff; text-decoration: underline;'>contracting@mahce.net</span></span></p>"
                    strMessage += "<br /><br />"
    ...
    Sub sendemail(ByVal emailstatus As String)
        Try
            ...
            'Changed by mohamed on 10/10/2020 as per discussion with natraj & parameswaran
            'If chkCumulative.Trim() = "CUMULATIVE" Then
            ...
            If headerDt.Rows.Count > 0 Then
                ...
                If Emailmode = "Test" Then
                    If (headerDt.Rows(0)("printheader").ToString.Trim = "INVOICE" Or headerDt.Rows(0)("printheader").ToString.Trim = "PROFORMA INVOICE") Then
                        status = "Confirmed"
                    Else
                        status = "On Request"
                    End If

                    '  status = IIf(headerDt.Rows(0)("printheader").ToString.Trim = "INVOICE", "Confirmed", "On Request")
                Else
                    If (headerDt.Rows(0)("printheader").ToString.Trim = "INVOICE" Or headerDt.Rows(0)("printheader").ToString.Trim = "PROFORMA INVOICE") Then
                        status = "Confirmed"
                    Else
                ...
                If Emailmode = "Test" Then
                    If (headerDt.Rows(0)("printheader").ToString.Trim = "INVOICE" Or headerDt.Rows(0)("printheader").ToString.Trim = "PROFORMA INVOICE") Then
                        ...
                        strSubject1 = AgentName & " - " & "Confirmation(" & hdFinalReqestId.Value & ")"
                    Else
                        'strSubject1 = AgentName & " - " & IIf(headerDt.Rows(0)("printheader").ToString.Trim = "PROFORMA INVOICE", "Confirmation(" & hdFinalReqestId.Value & ")", "Request(" & hdFinalReqestId.Value & ")")
                        strSubject1 = AgentName & " - " & "Request(" & hdFinalReqestId.Value & ")"
            ...
            param(0) = New SqlParameter("@bookingOrQuote", "BOOKING")
            param(1) = New SqlParameter("@bookingType", CType(chkCumulative, String))
            ...
            Dim bookingCcEmailRequired As Boolean = False
            Dim salesforceEmailId As String = ""
            Dim commonEmailId As String = ""
            If dtEmailConfigure.Rows.Count > 0 Then
                roEmailRequired = CType(dtEmailConfigure(0)("roEmail"), Boolean)
                agentEmailRequired = CType(dtEmailConfigure(0)("agentEmail"), Boolean)
                bookingCcEmailRequired = CType(dtEmailConfigure(0)("CcEmail"), Boolean)
            ...
            If agentEmailRequired = False Then
                agentemail = ""
            End If
            If bookingCcEmailRequired = False Then
                ccemails = ""
            End If
    ...
    Sub final_save()
        ...
        If dt.Rows.Count > 0 Then
            ...
            If chkColRef.Checked = True Then
                objBLLguest.GBColumbusref = txtColumbusReference.Text.Replace("/", "1.0/")
                Dim isBooking As String = objclsUtilities.ExecuteQueryReturnStringValue("select count(*)cnt from booking_header(nolock) where requestid='" + objBLLguest.GBColumbusref + "'")
                If isBooking > 0 Then
                    MessageBox.ShowMessage(Page, MessageType.Warning, "The same booking " & txtColumbusReference.Text & " is already exist")
                End If

            Else
                objBLLguest.GBColumbusref = ""
            End If


        End If

        objBLLguest.GBPayBookingRef = ""
        If Not Session("sEditRequestId") Is Nothing Then
            objBLLguest.GBBookmode = "EDIT"

            If Session("sEditRequestId").ToString.Contains("QR/") Or Session("sEditRequestId").ToString.Contains("QG/") Or Session("sEditRequestId").ToString.Contains("QP/") Or Session("sEditRequestId").ToString.Contains("TP/") Then
                objBLLguest.GBBookmode = "NEW"
                Dim PayBookingRef As String = objclsUtilities.ExecuteQueryReturnStringValue("select isnull(requestid,'') as requestid from booking_payment_requestHeader where temprequestid='" + Session("sEditRequestId") + "'")
                If Not PayBookingRef Is Nothing Then
                    objBLLguest.GBPayBookingRef = PayBookingRef
                End If
            Else
                Dim isBooking As String = objclsUtilities.ExecuteQueryReturnStringValue("select count(*)cnt from booking_header(nolock) where requestid='" + Session("sEditRequestId") + "'")
                If isBooking = 0 Then
        ...
        If strrequestid <> "" Then
            ...
            divSubmitBooking.Style.Add("display", "none")
            divabondon.Style.Add("display", "none")
            divPayLater.Style.Add("display", "none")
            divPayNow.Style.Add("display", "none")
            divPrintItinerary.Style.Add("display", "block")
            ...
            'added param 19/10/2020
            dvPaymentDetails.Style.Add("display", "none")
            dvPaymentStatus.Style.Add("display", "none")
            PaymentStatus.Text = ""
            'Changed by mohamed on 18/10/2020
            ' '' Email Sent hotel & Agent
            'Try
            '    hotelmail()
            'Catch ex As Exception
            ...
            'added param 04/09/2020
            Dim sqlparam(0) As SqlParameter
            sqlparam(0) = New SqlParameter("@requestId", CType(strrequestid, String))
            Dim bookingStatusDt As DataTable = objclsUtilities.GetDataTable("sp_get_hotelBookingStatus", sqlparam)
            If bookingStatusDt.Rows.Count > 0 Then
                dvBookingStatus.Visible = True
                gvBookingStatus.DataSource = bookingStatusDt
            ...
            End If
            FinalSaveExcursionInventory(strrequestid)
        Else
            MessageBox.ShowMessage(Page, MessageType.Errors, "Booking Failed  Please Submit Again ")

            divSubmitBooking.Style.Add("display", "block")
            divabondon.Style.Add("display", "block")
            'divPayLater.Style.Add("display", "block")
            'divPayNow.Style.Add("display", "block")
            divPrintItinerary.Style.Add("display", "none")
    ...
    Protected Sub btnSubmitBooking_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmitBooking.Click
        Try
            ...
            If chkColRef.Checked = True And txtColumbusReference.Text = "" Then
                MessageBox.ShowMessage(Page, MessageType.Warning, "Please enter 1.0 Invoice No.")
                dvColumbusReference.Visible = True
                chkColRef.Checked = False
                Exit Sub
            End If
    ...
    Protected Function submitBookingValidation() As Boolean
        Try
            '*** Danny 12/08/2018
            ...
            If ds.Tables(0).Rows.Count > 0 Or ds.Tables(1).Rows.Count > 0 Then
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    If Session("sRequestId").ToString.Contains("QR/") Or Session("sRequestId").ToString.Contains("QG/") Or Session("sRequestId").ToString.Contains("RP1.0/") Or Session("sRequestId").ToString.Contains("RPT1.0/") Or Session("sRequestId").ToString.Contains("RG1.0/") Or Session("sRequestId").ToString.Contains("RGT1.0/") Then
            ...
            'added param on 13/08/2020 --check availability and price in onedmc
            Dim priceCheckDt As DataTable = fnCheckAvailabilityAndPriceDMC()
            If Not priceCheckDt Is Nothing AndAlso priceCheckDt.Rows.Count > 0 Then
                lblAvailabilityTitle.Text = "Hotel Availability and Price Errors"
                Session("checkAvailabilityDt") = priceCheckDt
                gvCheckAvailability.DataSource = priceCheckDt
                gvCheckAvailability.DataBind()
                mpCheckAvailability.Show()
                Return False
            End If
    ...
    Protected Sub submitBooking()
        Try

            If submitBookingValidation() = False Then
                Exit Sub
            Else

                Session("APIsuccessCount") = 0

                'hotel Cancellation
                Dim cancelDt As DataTable = HotelCancellation()
                If Not cancelDt Is Nothing AndAlso cancelDt.Rows.Count > 0 Then
                    lblAvailabilityTitle.Text = "Hotel booking Cancellation Errors"
                    btnContinueBooking.Visible = False
                    gvCheckAvailability.Columns(4).Visible = False
                    gvCheckAvailability.Columns(5).Visible = False
                    gvCheckAvailability.DataSource = cancelDt
                    gvCheckAvailability.DataBind()
                    mpCheckAvailability.Show()
                    Exit Sub
                End If
                ...
                'Confirm hotel
                Dim confirmStatus As DataTable = ConfirmHotelBooking()
                If Not confirmStatus Is Nothing AndAlso confirmStatus.Rows.Count > 0 Then
                    lblAvailabilityTitle.Text = "Hotel booking Confirmation Errors"
                    gvCheckAvailability.Columns(4).Visible = True
                    gvCheckAvailability.Columns(5).Visible = True
                    Session("checkAvailabilityDt") = confirmStatus
                    gvCheckAvailability.DataSource = confirmStatus
                    gvCheckAvailability.DataBind()
                    mpCheckAvailability.Show()
                    Exit Sub
                End If

                ' Added by abin on 20201019
                Dim dt As DataTable
                Dim strAgentCode As String = ""
                dt = objBLLCommonFuntions.GetBookingTempHeaderDetails(Session("sRequestId"))
                If dt.Rows.Count > 0 Then
                    strAgentCode = dt.Rows(0)("agentcode").ToString
                End If

                ''Excursion inventory - Cancel
                Dim iExcInventoryCancelStatus As Integer
                iExcInventoryCancelStatus = CancelExcursionInventory(Session("sRequestId"), strAgentCode)

                'Excursion inventory- Confirm
                Dim iExcInventoryConfirmStatus As Integer
                iExcInventoryConfirmStatus = ConfirmExcursionInventory(Session("sRequestId"), strAgentCode)
    ...
    Sub hotelmail()
        ...
        If Not Session("sEditRequestId") Is Nothing Then
            If Session("sEditRequestId").ToString.Contains("QG") Or Session("sEditRequestId").ToString.Contains("QR") Or Session("sLoginType") = "RO" Then
                ...
                & " isnull(h.cancelled,0) cancelled from booking_hotel_detail d(nolock),booking_hotel_detail_confcancel h(nolock) where d.requestid=h.requestid and d.rlineno=h.rlineno and " _
                & " d.requestid='" & requestid & "' and (isnull(d.Rateplansource,'')='' or  d.Rateplansource='Columbus') "  'added param 07/10/2020
            Else
                sqlstr = "select  distinct d.partycode,isnull(h.amended,0)  amended,  " _
               & " isnull(h.cancelled,0) cancelled from booking_hotel_detail d(nolock),booking_hotel_detail_confcancel h(nolock) where d.requestid=h.requestid and d.rlineno=h.rlineno and " _
               & " d.requestid='" & requestid & "' and ( isnull(amended,0)=1 or isnull(cancelled,0)=1 or convert(varchar(10),d.moddate,111)=convert(varchar(10),getdate(),111) or  isnull(convert(varchar(10),d.moddate,111),'')='' ) " _
    ...
    Protected Sub btnproceed_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnproceed.Click
        Try
            ...
            ''Excursion inventory - Cancel
            Dim iExcInventoryCancelStatus As Integer
            iExcInventoryCancelStatus = CancelExcursionInventory(objBLLguest.GBRequestid, objBLLguest.GBAgentcode)

            'Excursion inventory- Confirm
            Dim iExcInventoryConfirmStatus As Integer
            iExcInventoryConfirmStatus = ConfirmExcursionInventory(objBLLguest.GBRequestid, objBLLguest.GBAgentcode)
...
<System.Web.Services.WebMethod()> _
    ...
    Protected Function fnCheckAvailabilityAndPriceDMC() As DataTable
        Try
            Dim dtHotel As New DataTable
            dtHotel = objBLLCommonFuntions.GetTempHotelDetailsWithAPIDetails(Session("sRequestId"))
            Dim errDt As DataTable = errorDataTable()
            If Not dtHotel Is Nothing Then
                If dtHotel.Rows.Count > 0 Then
                    For Each dr As DataRow In dtHotel.Rows
                        Dim result As APIHotelDetailsResponse.ResponseStatus = objApiController.CallHotelDetailsAPI_CheckPriceAndAvailability(dr, Session("GlobalUserName"))
                        Dim errDr As DataRow = errDt.NewRow
                        errDr("rlineNo") = dr("rlineno")
                        errDr("hotelName") = dr("hotelName") + ", " + Convert.ToDateTime(dr("checkin")).ToString("dd/MM/yyyy") + "-" + Convert.ToDateTime(dr("checkout")).ToString("dd/MM/yyyy")
                        errDr("roomName") = dr("roomName")
                        errDr("errMsg") = result.StatusDescription
                        errDr("statusCode") = result.StatusCode
                        errDr("ShowRemoveFlag") = 1
            ...
            If errDt.Rows.Count > 0 Then
                Dim errdtCopy As DataTable = errDt.Copy()
                Dim dv As DataView = errdtCopy.DefaultView
                dv.RowFilter = "statusCode<>'200'"
                dt = dv.ToTable()
                'Dim tt = (From n In errDt.AsEnumerable() Where n.Field(Of String)("statusCode") <> "200" Select n)
                If dt.Rows.Count > 0 Then
                    dt.TableName = "errorList"
                    Dim xmlerror As String = objclsUtils.GenerateXML(dt)
                    Dim param As New List(Of SqlParameter)
                    param.Add(New SqlParameter("@requestid", Session("sRequestId")))
                    param.Add(New SqlParameter("@xmlResult", CType(xmlerror, String)))
                    objclsUtilities.ExecuteNonQuery_Param("sp_update_booking_hotelDetailApiTemp", param)
                End If
                btnContinueBooking.Visible = True
            End If
            If dt.Rows.Count > 0 Then
                Return errDt
            Else
    ...
    Protected Sub btnRemoveBooking_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            Dim gvr As GridViewRow = CType(CType(sender, Control).Parent.Parent, GridViewRow)
            Dim lblRlineNo As Label
            lblRlineNo = CType(gvr.FindControl("lblRlineNo"), Label)
            objBLLguest.GBRequestid = Session("sRequestId")
            objBLLguest.GBRlineno = lblRlineNo.Text
            objBLLguest.GBErrorStatus = "Cancelled"
            objBLLguest.GBErrorStatusDescription = "Cancelled"
            objBLLguest.GBuserlogged = Session("GlobalUserName")
            objBLLguest.GBBookingStatus = "Cancelled"
            Dim result As Boolean = objBLLguest.UpdateConnectivityBookingFailedConfirmationTemp()
            If result = True Then
                Dim checkAvailabilityDt As DataTable = CType(Session("checkAvailabilityDt"), DataTable)
                Dim dv As DataView = checkAvailabilityDt.DefaultView
                dv.RowFilter = "rlineno<>" & lblRlineNo.Text
                Dim dt As DataTable = dv.ToTable
                gvCheckAvailability.DataSource = dt
                gvCheckAvailability.DataBind()
                Session("checkAvailabilityDt") = dt
            End If
            mpCheckAvailability.Show()
        Catch ex As Exception
            MessageBox.ShowMessage(Page, MessageType.Errors, "Error Description " + ex.Message.Replace("'", " "))
            objclsUtilities.WriteErrorLog("GuestPagenew.aspx :: btnEditBooking_Click :: " & ex.StackTrace & ":: " & Session("GlobalUserName"))
            mpCheckAvailability.Show()
        End Try
    End Sub

    Protected Function ConfirmHotelBooking() As DataTable
        Try
            btnContinueBooking.Visible = True
            Dim statusDt As DataTable = errorDataTable()
            Dim dtHotel As New DataTable
            dtHotel = objBLLCommonFuntions.GetTempHotelDetailsWithAPIDetails(Session("sRequestId"))
            Dim strPath As String = Server.MapPath("~\SavedApiDetails\")
            If Not dtHotel Is Nothing Then
                If dtHotel.Rows.Count > 0 Then
                    Dim Tries As Integer? = Convert.ToInt32(objclsUtilities.ExecuteQueryReturnSingleValue("select isnull(option_selected,1) from reservation_parameters where param_id =5504"))
                    If Tries Is Nothing AndAlso Tries < 1 Then Tries = 1
                    Dim confirmCnt As Integer = CType(Session("APIsuccessCount"), Integer)
                    Dim PendingConfirmCnt As Integer = dtHotel.Rows.Count
                    Dim dtguest As New DataTable
                    Dim paramList(0) As SqlParameter
                    paramList(0) = New SqlParameter("@requestId", CType(Session("sRequestId"), String))
                    dtguest = objclsUtilities.GetDataTable("sp_get_booking_guesttemp", paramList)
                    dtHotel.Columns.Add(New DataColumn("netPrice", GetType(Decimal)))
                    For Each dr As DataRow In dtHotel.Rows
                        Dim dv As DataView = dtguest.DefaultView
                        dv.RowFilter = "requestid = '" & dr("requestid") & "' and rlineno=" & dr("rlineno")
                        Dim guest As New DataTable
                        If dv.Count > 0 Then
                            guest = dv.ToTable()
                        End If
                        Dim result As APIHotelConfirmResponse.ResponseStatus = objApiController.CallHotelConfirmation(dr, strPath, guest, txtAgentReference.Text, Session("GlobalUserName"), Tries)

                        'Dim result As APIHotelConfirmResponse.ResponseStatus = New APIHotelConfirmResponse.ResponseStatus()
                        'result.StatusCode = "0005"
                        'result.StatusDescription = "Booking price has changed"
                        If result.StatusCode = System.Net.HttpStatusCode.OK Then
                            confirmCnt = confirmCnt + 1
                        Else
                            If confirmCnt > 0 Then
                                objBLLguest.GBRequestid = Session("sRequestId")
                                objBLLguest.GBRlineno = dr("rlineno")
                                objBLLguest.GBErrorStatus = "FAIL"
                                objBLLguest.GBErrorStatusDescription = result.StatusCode + " : " + result.StatusDescription
                                objBLLguest.GBuserlogged = Session("GlobalUserName")
                                objBLLguest.GBBookingStatus = "NotConfirmed"
                                objBLLguest.UpdateConnectivityBookingFailedConfirmationTemp()
                                Continue For
                            Else
                                Dim showRemoveFlag As Integer = 1
                                Dim checkDt As New DataTable

                                If result.StatusCode = "0005" Then
                                    btnContinueBooking.Visible = True
                                    If PendingConfirmCnt = 1 Then
                                        showRemoveFlag = 0
                                    Else
                                        showRemoveFlag = 1
                                    End If
                                Else
                                    ...
                                    End If
                                End If
                                Dim statusDr As DataRow = statusDt.NewRow
                                statusDr("rlineno") = dr("rlineno")
                                statusDr("hotelname") = dr("hotelName") + ", " + Convert.ToDateTime(dr("checkin")).ToString("dd/MM/yyyy") + "-" + Convert.ToDateTime(dr("checkout")).ToString("dd/MM/yyyy")
                                statusDr("roomname") = dr("roomName")
                                statusDr("errmsg") = result.StatusDescription
                                statusDr("statusCode") = result.StatusCode
                                statusDr("ShowRemoveFlag") = showRemoveFlag
                                statusDt.Rows.Add(statusDr)
                                Return statusDt
                                Exit Function
                            End If
                        End If
    ...
    Protected Sub btnEditBooking_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEditBooking.Click
        Try
            If Session("sRequestId") Is Nothing Then
                MessageBox.ShowMessage(Page, MessageType.Warning, "You have no booking.")
            Else
    ...
    Protected Sub btnContinueBooking_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnContinueBooking.Click
        Try
            Dim lblRlineNo As Label
            Dim lblStatusCode As Label
            Dim priceChangeFlag As Boolean = False
            For Each gvr As GridViewRow In gvCheckAvailability.Rows
                lblRlineNo = CType(gvr.FindControl("lblRlineNo"), Label)
                lblStatusCode = CType(gvr.FindControl("lblStatusCode"), Label)
                Dim chkPriceChange As CheckBox = CType(gvr.FindControl("chkPriceChange"), CheckBox)
                If lblStatusCode.Text = "0005" Then
                    If chkPriceChange.Checked = False Then
                        MessageBox.ShowMessage(Page, MessageType.Errors, "Select any one of the options, Search hotel again or Remove hotel or Book with revised price")
                        mpCheckAvailability.Show()
                        Exit Sub
                    Else
                        If priceChangeFlag = False Then priceChangeFlag = True
                    End If
                Else
                    If lblStatusCode.Text <> "200" Then
                        MessageBox.ShowMessage(Page, MessageType.Errors, "Select any one of the options, Search hotel again or Remove hotel")
                        mpCheckAvailability.Show()
                        Exit Sub
                    End If
                End If
            ...
            If dtHotel.Rows.Count > 0 Then
                For Each gvr As GridViewRow In gvCheckAvailability.Rows
                    ...
                    lblStatusCode = CType(gvr.FindControl("lblStatusCode"), Label)
                    Dim chkPriceChange As CheckBox = CType(gvr.FindControl("chkPriceChange"), CheckBox)
                    If lblStatusCode.Text = "0005" And chkPriceChange.Checked = True Then
                        Dim dv As DataView = dtHotel.DefaultView
                        dv.RowFilter = "rlineno=" & lblRlineNo.Text
                        Dim dt As DataTable = dv.ToTable
                        Dim result As String = objApiController.CallHotelDetailsAPI_updateResponse(dt, Session("GlobalUserName"))
                    End If
                Next
            End If
            mpCheckAvailability.Hide()
    ...
    Protected Function HotelCancellation() As DataTable
        Try
            btnContinueBooking.Visible = False
            Dim statusDt As DataTable = errorDataTable()
            Dim dtCancel As New DataTable
            Dim result As APIHotelCancellationResponse.ResponseStatus
            Dim sqlparam(0) As SqlParameter
            sqlparam(0) = New SqlParameter("@requestId", CType(Session("sRequestId"), String))
            dtCancel = objclsUtilities.GetDataTable("sp_get_booking_hotelTemp_Cancel_request", sqlparam)
            Dim strPath As String = Server.MapPath("~\SavedApiHotelCancellation\")
            If dtCancel.Rows.Count > 0 Then
                Dim Tries As Integer? = Convert.ToInt32(objclsUtilities.ExecuteQueryReturnSingleValue("select isnull(option_selected,1) from reservation_parameters where param_id =5505"))
                If Tries Is Nothing AndAlso Tries < 1 Then Tries = 1
                Dim confirmCnt As Integer = CType(Session("APIsuccessCount"), Integer)
                For Each dr As DataRow In dtCancel.Rows
                    result = objApiController.CallHotelCancellation(dr, strPath, Session("GlobalUserName"), Tries)
                    If result.StatusCode = System.Net.HttpStatusCode.OK Then
                        confirmCnt = confirmCnt + 1
                    Else
                        If confirmCnt > 0 Then
                            ...
                            objBLLguest.GBErrorStatusDescription = result.StatusCode + " : " + result.StatusDescription
                            objBLLguest.GBuserlogged = Session("GlobalUserName")
                            objBLLguest.UpdateConnectivityBookingFailedCancellationTemp()
                            Continue For
                        Else
                            Dim statusDr As DataRow = statusDt.NewRow
                            statusDr("rlineno") = dr("rlineno")
                    ...
                    End If
                Next
                Session("APIsuccessCount") = confirmCnt
            End If
            statusDt = Nothing
            Return statusDt
        Catch ex As Exception
    ...
    Protected Function errorDataTable() As DataTable
        Dim statusDt As New DataTable
        statusDt.Columns.Add(New DataColumn("rlineNo", GetType(Integer)))
        statusDt.Columns.Add(New DataColumn("hotelName", GetType(String)))
        statusDt.Columns.Add(New DataColumn("roomName", GetType(String)))
        statusDt.Columns.Add(New DataColumn("errmsg", GetType(String)))
        statusDt.Columns.Add(New DataColumn("statusCode", GetType(String)))
        statusDt.Columns.Add(New DataColumn("ShowRemoveFlag", GetType(Integer)))
        Return statusDt
    End Function

    Protected Sub gvCheckAvailability_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvCheckAvailability.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim gvr As GridViewRow = CType(e.Row, GridViewRow)
            Dim lblRlineNo As Label = CType(gvr.FindControl("lblRlineNo"), Label)
            Dim lblStatusCode As Label = CType(gvr.FindControl("lblStatusCode"), Label)
            Dim btnRemoveBooking As Button = CType(gvr.FindControl("btnRemoveBooking"), Button)
            Dim lblShowRemoveFlag As Label = CType(gvr.FindControl("lblShowRemoveFlag"), Label)
            Dim chkPriceChange As CheckBox = CType(gvr.FindControl("chkPriceChange"), CheckBox)
            Dim imgStatus As System.Web.UI.WebControls.Image = CType(gvr.FindControl("imgStatus"), System.Web.UI.WebControls.Image)
            Dim imgStatusNo As System.Web.UI.WebControls.Image = CType(gvr.FindControl("imgStatusNo"), System.Web.UI.WebControls.Image)
            If lblStatusCode.Text = "200" Then
                btnRemoveBooking.Visible = False
                chkPriceChange.Visible = False
                imgStatus.Visible = True
                imgStatusNo.Visible = False
            ElseIf lblStatusCode.Text = "0005" Then
                imgStatus.Visible = False
                imgStatusNo.Visible = True
                chkPriceChange.Visible = True
                If CType(lblShowRemoveFlag.Text, Integer) = 1 Then
    ...
    Protected Sub gvBookingStatus_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvBookingStatus.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim gvr As GridViewRow = CType(e.Row, GridViewRow)
            Dim lblCancelled As Label = CType(gvr.FindControl("lblCancelled"), Label)
            Dim imgStatus As System.Web.UI.WebControls.Image = CType(gvr.FindControl("imgStatus"), System.Web.UI.WebControls.Image)
            Dim imgStatusNo As System.Web.UI.WebControls.Image = CType(gvr.FindControl("imgStatusNo"), System.Web.UI.WebControls.Image)
            If lblCancelled.Text = "0" Then
                imgStatus.Visible = True
                imgStatusNo.Visible = False
            Else
                imgStatus.Visible = False
    ...
    Protected Sub btnPrintItinerary_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrintItinerary.Click
        Try
            Dim requestid As String = hdFinalReqestId.Value.Replace(",", "")
        ...
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objclsUtilities.WriteErrorLog("GuestPageNew.aspx :: btnPrintItinerary_Click :: " & ex.StackTrace & ":: " & Session("GlobalUserName"))
        End Try
    End Sub

    Protected Sub btnPayNow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPayNow.Click
        Try
            Dim reqAmt As String() = lbltotalbooking.Text.Split(" ")
            Dim requestAmount As Decimal = 0
            If Not reqAmt Is Nothing Then
                requestAmount = Math.Round(Convert.ToDecimal(reqAmt(0)), 2)
            End If
            If requestAmount - Math.Round(Convert.ToDecimal(hdnPaidAmount.Value), 2) <= 0 Then
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('The payment has already been done' );", True)
                Exit Sub
            End If
            requestAmount = requestAmount - Math.Round(Convert.ToDecimal(hdnPaidAmount.Value), 2)
            If submitBookingValidation() = False Then
                Exit Sub
            End If
            Dim requestId As String = createQuote()
            If requestId <> "" Then
                ...
                If Not Session("sEditRequestId") Is Nothing Then
                    Dim isBooking As String = objclsUtilities.ExecuteQueryReturnStringValue("select count(*)cnt from booking_header(nolock) where requestid='" + Session("sEditRequestId") + "'")
                    If isBooking > 0 Then
                        bookingNo = CType(Session("sEditRequestId"), String)
                        Session("payRequestid") = bookingNo
                    End If
                    Session("sEditRequestId") = requestId
                End If
                If Not Session("payRequestid") Is Nothing AndAlso Session("payRequestid") <> "" Then
                    bookingNo = Session("payRequestid")
                End If
                Session("PaymentReverseFlag") = "0"

                Dim userlogged As String = Session("GlobalUserName")
                Dim paymentRequestId As String = objBLLguest.GeneratePaymentRequestId(requestId, requestAmount, userlogged, bookingNo, "PAYNOW", "PAYMENT", "")
                Dim paymentUrl As String = ConfigurationManager.AppSettings("PaymentGatewayUrl").ToString
                Response.Redirect(paymentUrl + "?Id=" + paymentRequestId + "&PayNow=1", False)
            End If
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objclsUtilities.WriteErrorLog("GuestPageNew.aspx :: btnPayNow_Click :: " & ex.StackTrace & ":: " & Session("GlobalUserName"))
        End Try
    End Sub

    Protected Sub btnPayLater_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPayLater.Click
        Try
            Dim reqAmt As String() = lbltotalbooking.Text.Split(" ")
            Dim requestAmount As Decimal = 0
            If Not reqAmt Is Nothing Then
                requestAmount = Convert.ToDecimal(reqAmt(0))
            End If
            If requestAmount - Convert.ToDecimal(hdnPaidAmount.Value) <= 0 Then
                lblPayLaterMsg.Text = "The payment has already been done"
                btnPayLaterConfirm.Enabled = False
            Else
                lblPayLaterMsg.Text = ""
                btnPayLaterConfirm.Enabled = True
            End If
            requestAmount = requestAmount - Convert.ToDecimal(hdnPaidAmount.Value)

            If submitBookingValidation() = False Then
                Exit Sub
            End If
            Dim paramlist(0) As SqlParameter
            paramlist(0) = New SqlParameter("@requestId", Session("sRequestId"))
            Dim contactDt As DataTable = objclsUtilities.GetDataTable("sp_get_bookingEmail", paramlist)
            Dim emailId As String = ""
            If contactDt.Rows.Count > 0 Then
                emailId = Convert.ToString(contactDt.Rows(0)("salesemail"))
            End If
            hdnPayLaterAmount.Value = requestAmount
            lblPayLaterValue.Text = "Your booking value is " + Convert.ToString(Math.Round(requestAmount, 2)) + " " + reqAmt(1)
            lblPayLaterNote.Text = "You will receive an email link for payment in the " + emailId
            mpPayLater.Show()
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objclsUtilities.WriteErrorLog("GuestPageNew.aspx :: btnPayLater_Click :: " & ex.StackTrace & ":: " & Session("GlobalUserName"))
        End Try
    End Sub

    Protected Sub btnPayLaterConfirm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPayLaterConfirm.Click
        Try
            Dim requestId As String = createQuote()
            If requestId <> "" Then
                Session("sRequestId") = requestId
                ...
                End If
                Dim userlogged As String = Session("GlobalUserName")
                Dim paymentRequestId As String = objBLLguest.GeneratePaymentRequestId(requestId, requestAmount, userlogged, bookingNo, "PAYLATER", "PAYMENT", "")

                Dim paramlist(0) As SqlParameter
                paramlist(0) = New SqlParameter("@requestId", requestId)
                Dim ds As DataSet = objclsUtilities.GetDataSet("sp_get_payLater_details", paramlist)
                Dim headerDt As DataTable = ds.Tables(0)
                Dim contactDt As DataTable = ds.Tables(1)
                ...
                If headerDt.Rows.Count > 0 Then
                    ...
                    divcode = headerDt.Rows(0)("div_code")
                    bookingvalue = CType(headerDt.Rows(0)("salevalue"), String)
                    divName = headerDt.Rows(0)("division_master_des")

                    If Emailmode = "Test" Then
                        strSubject1 = AgentName & " - " & "Payment Requst(" & requestId & ")"

                    Else
                        strSubject1 = AgentName & " - " & "Payment Request(" & requestId & ")"
                    End If
                End If
                ...
                Else
                    ...
                    strMessage = "<center> <p class='MsoNormal' style='text-align: left;'><span style='font-family: calibri, sans-serif; color: #1B4F72; font-size: 12pt;'>Dear </span><span style='font-family: calibri, sans-serif; color: #ff7200; font-size: 12pt;'> " + contact + ",</span><span style='font-family:calibri,sans-serif;color:#1B4F72'><o:p /></span><br /></p><p class='MsoNormal'><b><span style='font-family:calibri,sans-serif;color:#002060'><o:p /></span></b></p></center>"
                End If
                strMessage += "<p class='MsoNormal' style='margin: 0'><br /></p> <p class='MsoNormal' style='margin: 0'><span style='font-family:calibri,sans-serif;color:#1B4F72'>Please find the below link for payment:</span></p>"

                Dim paymentUrl As String = ConfigurationManager.AppSettings("PaymentGatewayUrl").ToString
                paymentUrl = paymentUrl + "?Id=" + paymentRequestId

                strMessage = strMessage + "<p class='MsoNormal' style='margin: 0'><br /></p> <p class='MsoNormal' style='margin: 0'><span style='font-family:calibri,sans-serif;color:#1B4F72'><a href='" + paymentUrl + "' target='_blank'>" + paymentUrl + "</a></span></p>"

                strMessage += "  <br /><br />"
                strMessage += "<p class='MsoNormal' style='margin: 0'><br /></p> <p class='MsoNormal' style='margin: 0'><span style='font-family:calibri,sans-serif;color:#1B4F72'>Best Regards,</span></p>"
                strMessage += "<p class='MsoNormal' style='margin: 0'><br /></p> <p class='MsoNormal' style='margin: 0'><span style='font-family:calibri,sans-serif;color:#1B4F72'>" + divName + " </span></p>"
                ...
                Dim strpath1 As String = ""
                If clsEmail.SendEmailOnlinenew(strfromemailid, to_email, "", strSubject1, strMessage, strpath1, strfromusername, strfrompwd, lsSMTPAddress, lsPortNo) Then
                    objclsUtilities.SendEmailNotification("PAYMENTREQUEST", strfromemailid, to_email, agentemail, "", strSubject1, strMessage, "1", "1", strpath1, "Y", requestId, "MAIL_TO_AGENT")
                    ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Email sent successfully');", True)
                    btnMyBooking_Click(sender, e)
                Else
                    objclsUtilities.SendEmailNotification("PAYMENTREQUEST", strfromemailid, to_email, agentemail, "", strSubject1, strMessage, "1", "1", strpath1, "N", requestId, "EMAIL_TO_AGENT")
                    ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Email failed');", True)
                    mpPayLater.Show()
                End If
            End If
        Catch ex As Exception
    ...
    Protected Function createQuote() As String
        Try
            Dim dt As DataTable
            Dim divcode As String = ""
            Dim requestid As String = ""
            ...
            Dim ExistingQuoteId As String = ""
            dt = objBLLCommonFuntions.GetBookingTempHeaderDetails(Session("sRequestId"))
            If dt.Rows.Count > 0 Then
                divcode = dt.Rows(0)("div_code").ToString
                requestid = dt.Rows(0)("requestid").ToString
                agentRef = txtAgentReference.Text
                userlogged = Session("GlobalUserName")
                If Not Session("sEditRequestId") Is Nothing Then
                    strMode = "New"
                    If Session("sEditRequestId").ToString.Contains("QR/") Or Session("sEditRequestId").ToString.Contains("QG/") Or Session("sEditRequestId").ToString.Contains("QP/") Or Session("sEditRequestId").ToString.Contains("TP/") Then
                        strMode = "NEW"
                        quoteId = Session("sEditRequestId")
                    Else
                        Dim isBooking As String = objclsUtilities.ExecuteQueryReturnStringValue("select count(*)cnt from booking_header(nolock) where requestid='" + Session("sEditRequestId") + "'")
                        If isBooking = 0 Then
                            strMode = "NEW"
                        Else
                            Dim tempId As String = objclsUtilities.ExecuteQueryReturnStringValue("select top 1 isnull(tempRequestId,'') as tempRequestId from booking_payment_requestHeader where tempRequestId like 'QP/%' and requestid='" + Session("sEditRequestId") + "' order by id desc")
                            If tempId Is Nothing Or tempId = "" Then
                                strMode = "NEW"
                            Else
                                ExistingQuoteId = tempId
                                strMode = "EDIT"
                            End If
                        End If
                    End If
                Else
                    strMode = "New"
                End If
                Dim bookingNo As String = objclsUtilities.ExecuteQueryReturnStringValue("select top 1 isnull(RequestId,'') as tempRequestId from booking_payment_requestHeader where tempRequestId ='" + requestid + "' order by id desc")
                If Not bookingNo Is Nothing Then
                    Session("payRequestid") = bookingNo
                Else
                    Session("payRequestid") = ""
                End If
                quoteId = objBLLguest.FinalQuoteCreation(requestid, divcode, agentRef, strMode, quoteId, ExistingQuoteId)
            End If
            Return quoteId
        Catch ex As Exception
            Throw ex
    ...
    Private Function ConfirmExcursionInventory(ByVal strRequestId As String, ByVal Agentcode As String) As Integer
        Dim dtInventory As DataTable
        Dim iStatus As Integer = 1
        Try
            dtInventory = objclsUtilities.GetDataFromDataTable("select i.requestid,i.elineno,i.TEMP_CONF_NO,ei.InventoryId,i.TEMP_ID from booking_tours_inventory_temp(nolock)  i left outer join booking_tours_confcanceltemp(nolock) c  on c.requestid=i.requestid and c.elineno=i.elineno and isnull(c.cancelled,0)=0 ,excursionTypes_inventory(nolock) ei  where  isnull(i.PENDING_TO_BOOK,0)=1 and ei.exctypcode=case when type='NORMAL' then i.exctypcode else exctypcombocode end  and i.requestid='" + strRequestId + "' group by i.requestid,i.elineno,TEMP_CONF_NO,ei.InventoryId,i.TEMP_ID")
            If dtInventory.Rows.Count > 0 Then
                Dim objSubmitBooking As APIInventoryConfirm.SubmitBooking
                Dim objAPIInventoryController As New APIInventoryController
                Dim errdtCopy As DataTable = ExcInvErrorDataTable()
                For i As Integer = 0 To dtInventory.Rows.Count - 1
                    objSubmitBooking = New APIInventoryConfirm.SubmitBooking
                    objSubmitBooking.AgentID = Agentcode
                    objSubmitBooking.BookingID = dtInventory.Rows(i)("TEMP_ID").ToString
                    objSubmitBooking.ELineNo = dtInventory.Rows(i)("TEMP_ID").ToString 'dtInventory.Rows(i)("elineno").ToString
                    objSubmitBooking.InventoryID = dtInventory.Rows(i)("InventoryId").ToString
                    objSubmitBooking.TempConfirmationNo = dtInventory.Rows(i)("TEMP_CONF_NO").ToString
                    Dim objSubmitBookingResponse As New APIInventoryConfirm.SubmitBookingResponse
                    objSubmitBookingResponse = objAPIInventoryController.SubmitExcursionInventory(objSubmitBooking)

                    If objSubmitBookingResponse.Message = "Success" Then
                        Dim statusDr As DataRow = errdtCopy.NewRow
                        statusDr("elineno") = dtInventory.Rows(i)("elineno").ToString
                        statusDr("TEMP_CONF_NO") = dtInventory.Rows(i)("TEMP_CONF_NO").ToString
                        statusDr("InventoryId") = dtInventory.Rows(i)("InventoryId").ToString
                        statusDr("errmsg") = objSubmitBookingResponse.Message
                        statusDr("statusCode") = objSubmitBookingResponse.Message
                        statusDr("CONF_NO") = objSubmitBookingResponse.ConfirmationNo
                        errdtCopy.Rows.Add(statusDr)
                    Else
                        Dim statusDr As DataRow = errdtCopy.NewRow
                        statusDr("elineno") = dtInventory.Rows(i)("elineno").ToString
                        statusDr("TEMP_CONF_NO") = dtInventory.Rows(i)("TEMP_CONF_NO").ToString
                    ...
                    'Dim dv As DataView = errdtCopy.DefaultView
                    'dv.RowFilter = "statusCode<>'Success'"
                    'dt = dv.ToTable()
                    'Dim tt = (From n In errDt.AsEnumerable() Where n.Field(Of String)("statusCode") <> "200" Select n)
                ...
                If errdtCopy.Rows.Count > 0 Then
                    errdtCopy.TableName = "errorList"
                    Dim xmlerror As String = objclsUtils.GenerateXML(errdtCopy)
                    Dim param As New List(Of SqlParameter)
                    param.Add(New SqlParameter("@requestid", Session("sRequestId")))
                    param.Add(New SqlParameter("@xmlResult", CType(xmlerror, String)))
                    objclsUtilities.ExecuteNonQuery_Param("sp_update_booking_tours_inventoryTemp", param)
                End If
            Else
                Return iStatus
            End If
        ...
        Catch ex As Exception
            Return 0
            objclsUtilities.WriteErrorLog("GuestPageNew.aspx :: ConfirmExcursionInventory :: " & ex.StackTrace & ":: " & Session("GlobalUserName"))
        End Try
    End Function


    Protected Function ExcInvErrorDataTable() As DataTable
        Dim statusDt As New DataTable
        statusDt.Columns.Add(New DataColumn("elineno", GetType(Integer)))
        statusDt.Columns.Add(New DataColumn("TEMP_CONF_NO", GetType(String)))
        statusDt.Columns.Add(New DataColumn("InventoryId", GetType(String)))
        statusDt.Columns.Add(New DataColumn("errmsg", GetType(String)))
        statusDt.Columns.Add(New DataColumn("statusCode", GetType(String)))
        statusDt.Columns.Add(New DataColumn("CONF_NO", GetType(String)))
        Return statusDt
    End Function
    Protected Function ExcInvCancelDataTable() As DataTable
        Dim statusDt As New DataTable
        statusDt.Columns.Add(New DataColumn("elineno", GetType(Integer)))
        statusDt.Columns.Add(New DataColumn("CONF_NO", GetType(String)))
        statusDt.Columns.Add(New DataColumn("errmsg", GetType(String)))
        statusDt.Columns.Add(New DataColumn("statusCode", GetType(String)))
        statusDt.Columns.Add(New DataColumn("CANCEL_NO", GetType(String)))
        Return statusDt
    End Function

    Private Function CancelExcursionInventory(ByVal strRequestId As String, ByVal Agentcode As String) As Integer
        Dim dtInventoryCancel As DataTable
        Dim iStatus As Integer = 1
        Try
            dtInventoryCancel = objclsUtilities.GetDataFromDataTable("select distinct i.requestid,i.elineno,CONF_NO,TEMP_ID,ADULT,CHILD,SENIOR from booking_tours_inventory_temp(nolock) i inner join booking_tours_confcanceltemp(nolock) c on i.requestid=c.requestid and i.elineno=c.elineno and isnull(i.PENDING_TO_CANCEL,0)=1 and   isnull(c.cancelled,0)=1 where i.requestid='" + strRequestId + "' ")
            If dtInventoryCancel.Rows.Count > 0 Then
                Dim dtCancelCopy As DataTable = ExcInvCancelDataTable()
                Dim objCancelBooking As APIInventoryCancel.CancelBooking
                Dim objAPIInventoryController As New APIInventoryController
                For i As Integer = 0 To dtInventoryCancel.Rows.Count - 1
                    objCancelBooking = New APIInventoryCancel.CancelBooking
                    objCancelBooking.AgentID = Agentcode
                    objCancelBooking.ConfirmationNo = dtInventoryCancel.Rows(i)("CONF_NO").ToString
                    objCancelBooking.ReasonForCancellation = "Cancelled"


                    'objCancelBooking.NoOfAdults = dtInventoryCancel.Rows(i)("ADULT").ToString
                    'objCancelBooking.NoOfChildren = dtInventoryCancel.Rows(i)("CHILD").ToString
                    'objCancelBooking.NoOfSeniorCitizens = dtInventoryCancel.Rows(i)("SENIOR").ToString
                    'objCancelBooking.NoOfUnits = 0
                    Dim objTimeSlots As APIInventoryCancel.TimeSlot
                    Dim TimeSlots As List(Of APIInventoryCancel.TimeSlot) = New List(Of APIInventoryCancel.TimeSlot)

                    Dim dtInventoryCancelTimeSlot As DataTable
                    dtInventoryCancelTimeSlot = objclsUtilities.GetDataFromDataTable("select TIMESLOT_FROM,ADULT,CHILD,SENIOR from booking_tours_inventory_temp where requestid='" + strRequestId + "'  and elineno='" & dtInventoryCancel.Rows(i)("elineno").ToString & "' and CONF_NO='" & dtInventoryCancel.Rows(i)("CONF_NO").ToString & "'")

                    For j As Integer = 0 To dtInventoryCancelTimeSlot.Rows.Count - 1
                        objTimeSlots = New APIInventoryCancel.TimeSlot
                        'If objSaveAllotment.InventoryType = "ACS" Then
                        objTimeSlots.NoOfAdults = dtInventoryCancelTimeSlot.Rows(j)("ADULT")
                        objTimeSlots.NoOfChildren = dtInventoryCancelTimeSlot.Rows(j)("CHILD")
                        objTimeSlots.NoOfSeniorCitizens = dtInventoryCancelTimeSlot.Rows(j)("SENIOR")
                        ...
                        'End If
                        objTimeSlots.Time = dtInventoryCancelTimeSlot.Rows(j)("TIMESLOT_FROM")
                        TimeSlots.Add(objTimeSlots)
                    Next
                    objCancelBooking.TimeSlots = TimeSlots
                    Dim objCancelBookingResponse As New APIInventoryCancel.CancelBookingResponse
                    objCancelBookingResponse = objAPIInventoryController.CancelExcursionInventory(objCancelBooking)

                    If objCancelBookingResponse.Message = "Success" Then
                        Dim statusDr As DataRow = dtCancelCopy.NewRow
                        statusDr("elineno") = dtInventoryCancel.Rows(i)("elineno").ToString
                        statusDr("CONF_NO") = dtInventoryCancel.Rows(i)("CONF_NO").ToString
                        statusDr("errmsg") = objCancelBookingResponse.Message
                        statusDr("statusCode") = objCancelBookingResponse.Message
                        statusDr("CANCEL_NO") = objCancelBookingResponse.CancellationNo
                        dtCancelCopy.Rows.Add(statusDr)
                    Else
                        Dim statusDr As DataRow = dtCancelCopy.NewRow
                        statusDr("elineno") = dtInventoryCancel.Rows(i)("elineno").ToString
                        statusDr("CONF_NO") = dtInventoryCancel.Rows(i)("CONF_NO").ToString
                ...
                If dtCancelCopy.Rows.Count > 0 Then
                    dtCancelCopy.TableName = "CancelList"
                    Dim xmlerror As String = objclsUtils.GenerateXML(dtCancelCopy)
                    Dim param As New List(Of SqlParameter)
                    param.Add(New SqlParameter("@requestid", Session("sRequestId")))
                    param.Add(New SqlParameter("@xmlResult", CType(xmlerror, String)))
                    objclsUtilities.ExecuteNonQuery_Param("sp_cancel_booking_tours_inventoryTemp", param)
                End If
        ...
        Catch ex As Exception
            Return 0
            objclsUtilities.WriteErrorLog("GuestPageNew.aspx :: CancelExcursionInventory :: " & ex.StackTrace & ":: " & Session("GlobalUserName"))
        End Try
    ...
    Protected Sub ddlPaymentType_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPaymentType.SelectedIndexChanged
        ...
        If Paymenttype = "Refund" Then
            PaymentAmount.Attributes.Add("style", "display:block")
            PaymentAmount.Attributes.Add("style", "margin-top:20px")
            PayCurrency.Attributes.Add("style", "display:block")
            PayCurrency.Attributes.Add("style", "width:30px")
        Else
            PaymentAmount.Attributes.Add("style", "display:none")
            PayCurrency.Attributes.Add("style", "display:none")
        End If
    End Sub

    Protected Sub btnReverseRefund_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReverseRefund.Click

        Try
            If ddlPaymentType.SelectedValue = "Refund" Then
                If PaymentAmount.Text = "" Or Not IsNumeric(PaymentAmount.Text) Then
                    ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Please enter refund amount' );", True)
                    Exit Sub
                End If
                Dim amt As Decimal = Math.Round(Convert.ToDecimal(PaymentAmount.Text), 2)
                If amt > hdnPayAmount.Value Then
                    ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('refund amount is greater than booking amount' );", True)
                    Exit Sub
                End If
            End If
            Dim Amount As String = Math.Round(Convert.ToDecimal(PaymentAmount.Text), 2)
            Dim RefundPaymentRequestId As String = hdnPaymentRequestId.Value
            Dim Currency As String = hdnPayCurrency.Value
            ...
            If Paymenttype = "Refund" Then
                Dim paymentRequestId As String = objBLLguest.GeneratePaymentRequestId(requestid, Amount, userlogged, "", "", "REFUND", RefundPaymentRequestId)
                Session("PaymentReverseFlag") = "0"

                Dim RefundUrl As String = ConfigurationManager.AppSettings("RefundUrl").ToString
                Response.Redirect(RefundUrl + "?Id=" + PaymentId + "&Currency=" + Currency + "&Amount=" + Amount + "&merchantTransactionId=" + paymentRequestId, False)
            Else
                Dim paymentRequestId As String = objBLLguest.GeneratePaymentRequestId(requestid, Amount, userlogged, "", "", "REVERSE", RefundPaymentRequestId)
                Session("PaymentReverseFlag") = "1"

                Dim ReversalUrl As String = ConfigurationManager.AppSettings("ReversalUrl").ToString
                Response.Redirect(ReversalUrl + "?Id=" + PaymentId + "&Currency=" + Currency + "&merchantTransactionId=" + paymentRequestId, False)
            End If
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objclsUtilities.WriteErrorLog("GuestPageNew.aspx :: btnReverseRefund_Click :: " & ex.StackTrace & ":: " & Session("GlobalUserName"))
        End Try
    End Sub

    Protected Sub ddlBookingStatus_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlBookingStatus.SelectedIndexChanged

        Dim Paymenttype As String = ddlBookingStatus.SelectedValue
        If Paymenttype = "Booking Failed" Then
            DivRefundReverse.Attributes.Add("style", "display:block")
        Else
            DivRefundReverse.Attributes.Add("style", "display:none")
        End If
    End Sub

    Private Sub FinalSaveExcursionInventory(ByVal strrequestid As String)
        Dim dtInventory As DataTable
        Dim iStatus As Integer = 1
        Try
            dtInventory = objclsUtilities.GetDataFromDataTable("select distinct requestid,elineno,CONF_NO,TEMP_ID from booking_tours_inventory(nolock) where requestid='" & strrequestid & "' and isnull(CONF_NO,'') <>'' and  isnull(CANCEL_NO,'') ='' order by elineno ")
            If dtInventory.Rows.Count > 0 Then
                Dim objFinalSaveRequest As APIInventoryConfirm.FinalSaveRequest
                Dim objAPIInventoryController As New APIInventoryController
                Dim errdtCopy As DataTable = ExcInvErrorDataTable()
                For i As Integer = 0 To dtInventory.Rows.Count - 1
                    objFinalSaveRequest = New APIInventoryConfirm.FinalSaveRequest

                    objFinalSaveRequest.ColumbusBookingID = dtInventory.Rows(i)("requestid").ToString
                    objFinalSaveRequest.ColumbusTempBookingID = dtInventory.Rows(i)("TEMP_ID").ToString 'dtInventory.Rows(i)("elineno").ToString
                    objFinalSaveRequest.ELineNo = dtInventory.Rows(i)("elineno").ToString
                    objFinalSaveRequest.InventoryConfirmationNo = dtInventory.Rows(i)("CONF_NO").ToString
                    Dim objFinalSaveResponse As New APIInventoryConfirm.FinalSaveResponse
                    objFinalSaveResponse = objAPIInventoryController.FinalSaveExcursionInventory(objFinalSaveRequest)

                    If objFinalSaveResponse.Message = "Success" Then
                        'Success
                    Else
                        'Failed
                    End If
        ...
        Catch ex As Exception
            ...
            objclsUtilities.WriteErrorLog("GuestPageNew.aspx :: FinalSaveExcursionInventory :: " & ex.StackTrace & ":: " & Session("GlobalUserName"))
        End Try
