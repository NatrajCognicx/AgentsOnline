...
Partial Class PriceListDatewise
...
#Region "Web Services"
    ...
    Public Shared Function GetHotelName(ByVal prefixText As String) As List(Of String)
        ...
        Try
            strSqlQry = "select v.partycode,v.partyname from view_approved_hotels_new v(nolock) where v.partyname like  '%" & prefixText.Trim() & "%'"
   ...
   <System.Web.Services.WebMethod()> _
    ...
    <WebMethod()> _
    Public Shared Function GetHotelsList(ByVal contextKey As String) As String
        Dim strSqlQry As String = ""
        Dim myDS As New DataSet
        Dim Hotelnames As New List(Of String)
        Try
            ...
            'strSqlQry = "select v.partycode,v.partyname from sectormaster(nolock) s,catmast(nolock) c,view_approved_hotels_new v where v.sectorcode=s.sectorcode  and v.catcode=c.catcode  "
            ...
            strSqlQry = "select * from view_approved_hotels_new"
...
#Region "Protected Sub btnSearch_Click(sender As Object, e As System.EventArgs) Handles btnSearch.Click"
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        Try
            ...
            strNoOfNights = ""
            strNoOfNights = clsUtilities.SharedExecuteQueryReturnStringValue("select DATEDIFF(day,convert(datetime,'" & Convert.ToDateTime(txtFromDate.Text).ToString("yyyy/MM/dd") & "'),convert(datetime,'" & Convert.ToDateTime(txtToDate.Text).ToString("yyyy/MM/dd") & "'))")
            If strNoOfNights > 30 Then
                MessageBox.ShowMessage(Page, MessageType.Warning, "More than 30 days is not allowed.")
                Exit Sub
            End If
...
#End Region
    ...
    Private Sub BindHotelName()

        Dim strSqlQry As String = ""
        strSqlQry = "select v.partycode,lower(v.partyname)partyname from view_approved_hotels_new v(nolock)"
        strSqlQry = strSqlQry & " order by v.partyname"

        objclsUtilities.FillDropDownList(ddlHotelNames, strSqlQry, "partycode", "partyname", False, "--")


    End Sub

    Private Sub BindRoomClass()

        Dim strSqlQry As String = ""
        strSqlQry = "select distinct roomclasscode,roomclassname from room_classification(nolock)"

        objclsUtilities.FillDropDownList(ddlRoomClass, strSqlQry, "roomclasscode", "roomclassname", False, "--")


    End Sub

    Public Sub DownLoadRatesheet()
        ...
        table.Rows.Add("DATE", txtFromDate.Text & " to " & txtToDate.Text)
        table.Rows.Add("DESTINATION/LOCATION", txtDestinationName.Text.Trim)
        table.Rows.Add("AGENT NAME", txtCustomer.Text)
        table.Rows.Add("COUNTRY NAME", txtCountry.Text)
        table.Rows.Add("STAR CATEGORY", txtHotelStars.Text)
        table.Rows.Add("HOTELS", txtHotelCode.Text)
        table.Rows.Add("ROOM CLASSIFICATION", ddlRoomClass.SelectedItem.Text)
        table.Rows.Add("OFFER TYPE", ddlOfferType.SelectedValue)
        table.Rows.Add("OCCUPANCY TYPE", ddlOccupancy.SelectedValue)
        table.Rows.Add("RATE TYPE", ddlRateType.SelectedValue)
        gvHeading.DataSource = table
        gvHeading.DataBind()


        SqlConn = clsDBConnect.dbConnectionnew("strDBConnection")
        mySqlCmd = New SqlCommand("sp_rep_ratesheet_datewise", SqlConn)
        mySqlCmd.CommandType = CommandType.StoredProcedure
        mySqlCmd.Parameters.Add(New SqlParameter("@destinationtype", SqlDbType.VarChar, 20)).Value = strDestType.Trim
        mySqlCmd.Parameters.Add(New SqlParameter("@destinationcode", SqlDbType.VarChar, 20)).Value = strDestinationCode.Trim
        mySqlCmd.Parameters.Add(New SqlParameter("@starcatcode", SqlDbType.VarChar, 20)).Value = txtHotelStarsCode.Text.Trim
        mySqlCmd.Parameters.Add(New SqlParameter("@partycodes", SqlDbType.VarChar, 2000)).Value = txtHotelCode.Text.Trim
        ...
        mySqlCmd.Parameters.Add(New SqlParameter("@RateType", SqlDbType.VarChar, 100)).Value = ddlRateType.SelectedValue
        ...
        mySqlCmd.Parameters.Add(New SqlParameter("@rmcatcode", SqlDbType.VarChar, 20)).Value = ddlOccupancy.SelectedValue
        mySqlCmd.Parameters.Add(New SqlParameter("@roomclasscode", SqlDbType.VarChar, 20)).Value = ddlRoomClass.SelectedValue
        mySqlCmd.Parameters.Add(New SqlParameter("@offertype", SqlDbType.VarChar, 20)).Value = ddlOfferType.SelectedValue
...
#End Region
    ...
    Protected Sub btnfilter_Click(sender As Object, e As System.EventArgs) Handles btnfilter.Click
        ...
        If txtDestinationCode.Text.Trim <> "" Then
            Dim strDests() = txtDestinationCode.Text.Trim.Split("|")
            strDest = strDests(0)
        End If
        Dim strStar As String = txtHotelStarsCode.Text.Trim
    ...
    Private Sub ExportToExcel()
        Response.Clear()
        Response.Buffer = True
        Dim strFileName As String = "BestRateDayWise_" & Now.Year & Now.Month & Now.Day & Now.Hour & Now.Minute & Now.Second & Now.Millisecond & ".xls"
        ...
        Using sw As New StringWriter()
            ...
            hw.AddAttribute(HtmlTextWriterAttribute.Cellpadding, "3")
            hw.AddStyleAttribute(HtmlTextWriterStyle.FontSize, "Large")
            ...
            hw.Write("REPORT SEARCH FIELDS")
            hw.RenderEndTag()



            gvHeading.RenderControl(hw)
    ...
    Protected Sub GridView1_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        If (e.Row.RowType = DataControlRowType.DataRow) Then
            ...
            If e.Row.Cells(4).Text = "CONTRACT" Then
                e.Row.Cells(4).BackColor = Color.Green
            Else
