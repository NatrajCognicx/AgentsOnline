...
#Region "Public Sub GenerateReport(ByVal requestID As String, ByRef bytes() As Byte, ByRef ds1 As DataSet, ByVal printMode As String, objResParam As ReservationParameters, Optional ByVal fileName As String = "")"
    Public Sub GenerateReport(ByVal requestID As String, ByRef bytes() As Byte, ByRef ds1 As DataSet, ByVal printMode As String, ByVal objResParam As ReservationParameters, Optional ByVal fileName As String = "")
        Try
            ...
            Dim Showguestroomwise As String = objclsUtilities.ExecuteQueryReturnSingleValue("select option_selected from reservation_parameters where param_id=5304") '' Added shahul 25/07/18
                ...
                Using memoryStream As New System.IO.MemoryStream()
                    ...
                    If hotelDt.Rows.Count > 0 Then
                        ...
                        For Each hotelDr As DataRow In hotelDt.Rows
                            ...
                            If IsDBNull(hotelDr("cancelledBookingRlineno")) = False AndAlso Convert.ToInt32(hotelDr("cancelledBookingRlineno")) <> 0 Then
                                phrase.Add(New Chunk(Space(25) & "[ Hotel Cancellation Charges ]", NormalFontBold))
                                confText = vbCrLf & vbCrLf & "Hotel Cancellation No : "
                            Else
                                confText = "Hotel Conf No : "
                            End If
                            If Convert.ToString(hotelDr("hotelConfNo")) <> "" Then
                                phrase.Add(New Chunk(confText & Convert.ToString(hotelDr("hotelConfNo")), NormalFontBold))
                            End If
                    ...
                    If strBankImage <> "" Then
                        ...
                        cell = ImageCell("~/img/" & strBankImage, 500.0F, PdfPCell.ALIGN_CENTER)
                        tblBank.AddCell(cell)
                    ...
                    '    cell.PaddingBottom = 4
                    '    tblBank.AddCell(cell)
                    '    cell = ImageCell("~/img/RP_RAK Bank.jpg", 500.0F, PdfPCell.ALIGN_CENTER)
                    '    tblBank.AddCell(cell)
                    '    document.Add(tblBank)
                    ...
                    '    cell.PaddingBottom = 4
                    '    tblBank.AddCell(cell)
                    '    cell = ImageCell("~/img/RG_MashreqBank.jpg", 500.0F, PdfPCell.ALIGN_CENTER)
                    '    tblBank.AddCell(cell)
                    '    document.Add(tblBank)
                    'End If
...
#End Region
    Protected Function ServiceRemarksParce(ByVal otherDr As DataRow) As String
        Dim strRemarks As String = ""
        Try
            '*** Remarks Danny 07/10/2018

            'If otherDr("SupplierRemark").ToString.Trim.Length > 0 Then
            '    strRemarks = "    Supplier Remark:" + otherDr("SupplierRemark")
            'End If
            ...
            End If
            'If otherDr("ArrivalRemark").ToString.Trim.Length > 0 Then
            '    If (strRemarks.Trim.Length > 0) Then
            '        strRemarks = strRemarks + vbCr
            '    End If
            '    strRemarks = strRemarks + "    Arrival Remark:" + otherDr("ArrivalRemark")
            'End If
            'If otherDr("DepartureRemark").ToString.Trim.Length > 0 Then
            '    If (strRemarks.Trim.Length > 0) Then
            '        strRemarks = strRemarks + vbCr
            '    End If
            '    strRemarks = strRemarks + "    Departure Remark:" + otherDr("DepartureRemark")
            'End If
        Catch ex As Exception
...
#Region "Protected Sub AppendOtherServices(ByRef tblOthServ As PdfPTable, ByVal inputDt As DataTable)"
    Protected Sub AppendOtherServices(ByRef tblOthServ As PdfPTable, ByVal inputDt As DataTable)
        ...
        For Each inputDr As DataRow In inputDt.Rows
            ...
            phrase.Add(New Chunk(vbCr + Convert.ToString(inputDr("ServiceRemark")), SmallFont))
            ...
            '*** Remarks Danny 07/10/2018
            'If inputDr("ServiceRemark").ToString.Trim.Length > 0 Then
            '    phrase = New Phrase()
            '    phrase.Add(New Chunk(Convert.ToString(inputDr("ServiceRemark")), SmallFont))
            '    If Convert.ToString(inputDr("OthServType")) = "Transfer" Then
            '        cell = PhraseCell(phrase, PdfPCell.ALIGN_LEFT, 1, True, "Yes")
            '    Else
            '        cell = PhraseCell(phrase, PdfPCell.ALIGN_LEFT, 1, True)
...
#Region "Public Sub GenerateCumulativeReport(ByVal requestID As String, ByRef bytes() As Byte, ByRef ds1 As DataSet, ByVal printMode As String, ByVal objResParam As ReservationParameters, Optional ByVal fileName As String = "")"
    Public Sub GenerateCumulativeReport(ByVal requestID As String, ByRef bytes() As Byte, ByRef ds1 As DataSet, ByVal printMode As String, ByVal objResParam As ReservationParameters, Optional ByVal fileName As String = "")
        Try
            Dim sqlConn As New SqlConnection
            Dim mySqlCmd As New SqlCommand
            Dim myDataAdapter As New SqlDataAdapter
            ...
            If headerDt.Rows.Count > 0 Then
                ...
                Using memoryStream As New System.IO.MemoryStream()
                    ...
                    '    cell.PaddingBottom = 4
                    '    tblBank.AddCell(cell)
                    '    cell = ImageCell("~/img/RGT-Bank.png", 500.0F, PdfPCell.ALIGN_CENTER)
                    '    tblBank.AddCell(cell)
                    '    document.Add(tblBank)
                    'End If
...
#Region "Public Sub GenerateReportProformaVat(ByVal requestID As String, ByRef bytes() As Byte, ByRef ds1 As DataSet, ByVal printMode As String, Optional ByVal fileName As String = "")"
    ...
    Public Sub GenerateReportProformaVat(ByVal requestID As String, ByVal ManualInvNo As String, ByRef bytes() As Byte, ByRef ds1 As DataSet, ByVal printMode As String, ByVal objResParam As ReservationParameters, Optional ByVal fileName As String = "")
        Try
            Dim sqlConn As New SqlConnection
            Dim mySqlCmd As New SqlCommand
            Dim myDataAdapter As New SqlDataAdapter
            Dim ds As New DataSet
            sqlConn = clsDBConnect.dbConnectionnew("strDBConnection")
            Dim decimalplaces As Integer = objclsUtilities.ExecuteQueryReturnStringValue("(select option_selected from reservation_parameters where param_id=509)")
                        ...
                        For Each hotelDr As DataRow In hotelDt.Rows
                            ...
                            If filterTariffDt.Rows.Count > 0 Then
                                For Each ratesDr As DataRow In filterTariffDt.Rows
                                    ...
                                    If ratesDr("bookingCode") <> "" Then
                                        'phrase.Add(New Chunk(vbCrLf & "( " + Convert.ToString(ratesDr("bookingCode")) + " )", NormalFontBold))
                                        phrase.Add(New Chunk(vbCrLf & "( " + Convert.ToString(ratesDr("bookingCode")) + " )", NormalFontVAT))
                                    End If
                                Next
                            End If
                            cell = PhraseCell(phrase, PdfPCell.ALIGN_LEFT, 1, False)
...
#Region "Private Shared Function ImageCellvat(path As String, scale As Single, align As Integer) As PdfPCell"
    Private Shared Function ImageCellvat(ByVal path As String, ByVal height As Single, ByVal width As Single, ByVal align As Integer) As PdfPCell
        Dim image As iTextSharp.text.Image = iTextSharp.text.Image.GetInstance(HttpContext.Current.Server.MapPath(path))
        image.ScaleAbsolute(height, width)
        Dim cell As New PdfPCell(image)
        cell.BorderColor = BaseColor.WHITE
        ' cell.Border = Rectangle.BOTTOM_BORDER Or Rectangle.TOP_BORDER Or Rectangle.LEFT_BORDER
        cell.VerticalAlignment = PdfPCell.ALIGN_TOP
        cell.HorizontalAlignment = align
        cell.PaddingBottom = 0.0F
        cell.PaddingTop = 0.0F
